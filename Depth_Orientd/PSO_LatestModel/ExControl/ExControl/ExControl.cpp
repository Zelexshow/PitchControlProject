	...
	int *aviFail, char *accInfile, char *avcOutname, char *avcMsg)
{
	...
	//定义控制变量:转矩、三个桨距角、流速、一个保留区other
	double torque, pitch1, pitch2, pitch3, speed;// other
	...
	char  ControlBuf[100]; // 存放simulink发送过来的数据 
	char  WindBuf[100];  // 存放发往simulink的风机数据 
	...
	//sscanf(ControlBuf, "%lf;%lf;%lf;%lf;%lf;", &torque, &pitch1, &pitch2, &pitch3, &speed); //存收到的数据
	sscanf(ControlBuf, "%lf;%lf;%lf;%lf;", &torque, &pitch1, &pitch2, &pitch3);
																							//将模型参数传送给MyFunc								 
